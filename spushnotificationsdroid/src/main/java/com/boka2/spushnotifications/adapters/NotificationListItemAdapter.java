package com.boka2.spushnotifications.adapters;
...
public class NotificationListItemAdapter extends ArrayAdapter<SModPushNotification>{
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (v == null) {		
			...
			holder.txtDescription.setCustomFontByFontFile(SPushNotificationsStyleManager.Current(activity).getStyle().getCellStyle()
			...
			holder.txtDate.setCustomFontByFontFile(SPushNotificationsStyleManager.Current(activity).getStyle().getCellStyle().getStrDateFontFamily(),
					activity);
		...
		if (notif != null) {				
			...
			if("webview".equals(notif.strCode)) {
				holder.txtDescription.setCompoundDrawablesWithIntrinsicBounds(0, 0, R.drawable.icon_attach, 0);
			} else {
